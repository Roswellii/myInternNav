...
class InternVLAN1Net(PreTrainedModel):
    ...
    def __init__(self, config: Union[InternVLAN1ModelConfig, ModelCfg]):
        ...
        self.model = InternVLAN1ForCausalLM.from_pretrained(
            ...
            attn_implementation="sdpa", device_map={"": self.model_config.device}
        )
    ...
    def s2_step(self, rgb, depth, pose, instruction, intrinsic, look_down=False):
        ...
        parts = split_and_clean(prompt_instruction) # 将包含 <image> 标记的文本字符串分割成文本片段和 <image> 标记的列表，便于后续构建多模态内容。
        #例子： [  "You are an autonomous navigation assistant. Your task is to go to the kitchen. These are your historical observations:",  "<image>",  "<image>",  ".",  "you can see",  "<image>",  "."]
        
        # 形成交替的文本和图像内容，parts就是上面按照部分定义好的结构
        content = []
